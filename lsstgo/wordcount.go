...
// FUNC OMIT
func mostCommonWords(text string, topN int) []pair {
	uniqueWords := strings.Fields(text)
	...
	pairs := make([]pair, topN)
	for word, count := range wordCounts {
		if count <= pairs[topN-1].count {
			// Rarer than the least-common word, so move on.
			continue
		}

		// For small N, iterating is plenty fast.
	InsertLoop:
		for i, p := range pairs {
			if count > p.count {
				// Insert the pair and slide everything else down
				if i < topN-1 {
					copy(pairs[i+1:topN-1], pairs[i:topN-2])
				}
				pairs[i] = pair{word: word, count: count}
				break InsertLoop
			}
		}
	}
...
func main() {
	top20 := mostCommonWords(styleGuide, 20)
	for i, pair := range top20 {
...
E133
   ...
   This pycodestyle error_ (via flake8_) is not part of :pep:8.
...
The separate pep8-naming_ plugin validates names according to the DM Python Style Guide.
...
Many :pep:8 issues in existing code can be fixed with autopep8_ version 1.2 or newer:
...
When raising an exception in Python code, consideration should be given to defining a module-specific exception_ for increased precision.
